...
fn failure_message(name: &str) -> String {
    if normalized(name) == "awaitshell" {
        return "Tool \"AwaitShell\" is no longer available in this haxsd byok version. The model emitted a tool name that is not part of the current advertised tool set. Treat the tool call as failed and continue using only tools advertised in the current prompt; for background shell work, use the current Shell/background completion flow.".into();
    }
    format!(
        "Tool \"{name}\" is not available in this haxsd byok version. The model emitted a tool name that is not part of the current advertised tool set. Treat the tool call as failed and continue using a tool advertised in the current prompt."
    )
}
